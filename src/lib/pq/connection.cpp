...
namespace tao::pq
{
   ...
   auto basic_connection::error_message() const -> std::string
   {
   ...
   void basic_connection::check_prepared_name( const std::string& name )
   {
   ...
   auto basic_connection::is_prepared( const char* name ) const noexcept -> bool
   {
      return m_prepared_statements.find( name ) != m_prepared_statements.end();
   }

   auto basic_connection::execute_params( const char* statement,
                                          const int n_params,
                                          const Oid types[],
   ...
   basic_connection::basic_connection( const std::string& connection_info )
   ...
   auto basic_connection::is_open() const noexcept -> bool
   {
      return PQstatus( m_pgconn.get() ) == CONNECTION_OK;
   }

   void basic_connection::prepare( const std::string& name, const std::string& statement )
   {
   ...
   void basic_connection::deallocate( const std::string& name )
   {
      check_prepared_name( name );
